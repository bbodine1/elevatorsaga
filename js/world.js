var createWorldCreator = function () {
  ...
  creator.createElevators = function (
    elevatorCount,
    floorCount,
    floorHeight,
    elevatorCapacities
  ...
  creator.spawnUserRandomly = function (floorCount, floorHeight, floors) {
    ...
    if (currentFloor === 0) {
      ...
      if (_.random(10) === 0) {
        destinationFloor =
          (currentFloor + _.random(1, floorCount - 1)) % floorCount;
      } else {
        destinationFloor = 0;
      }
    }
  ...
  creator.createWorld = function (options) {
    ...
    world.floors = creator.createFloors(
      options.floorCount,
      world.floorHeight,
      handleUserCodeError
    );
    ...
    var registerUser = function (user) {
      ...
      user.on("exited_elevator", function () {
        ...
        world.maxWaitTime = Math.max(
          world.maxWaitTime,
          world.elapsedTime - user.spawnTimestamp
        );
        world.avgWaitTime =
          (world.avgWaitTime * (world.transportedCounter - 1) +
            (world.elapsedTime - user.spawnTimestamp)) /
          world.transportedCounter;
        recalculateStats();
      });
    ...
    var handleButtonRepressing = function (eventName, floor) {
      ...
      for (
        var i = 0, len = world.elevators.length, offset = _.random(len - 1);
        i < len;
        ++i
      ) {
    ...
    world.update = function (dt) {
      ...
      for (var users = world.users, i = 0, len = users.length; i < len; ++i) {
        ...
        world.maxWaitTime = Math.max(
          world.maxWaitTime,
          world.elapsedTime - u.spawnTimestamp
        );
      }
